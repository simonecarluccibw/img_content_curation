...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Generate AI image metadata per hotel from an ICEPortal CSV."
    )
...
def load_prompts(path: Path) -> Dict:
    ...
    if suffix in {".yaml", ".yml"}:
        if yaml is None:
            raise RuntimeError(
                "YAML prompts require PyYAML. Install it with `pip install PyYAML` or use a JSON prompts file."
            )
    ...
    for attempt in range(1, max_retries + 1):
        try:
            with urllib.request.urlopen(request, timeout=timeout) as response:
                ...
                parsed = json.loads(extract_json_text(response_data))
                return {
...
def process_hotel(
    ...
    for index, row in enumerate(rows, start=1):
        ...
        try:
            ...
            print(
                f"[ROW ERROR] {propid} image {index}/{len(rows)} | Asset_FileID={row.get('Asset_FileID', '')} | {exc}",
                file=sys.stderr,
            )
...
def main() -> int:
    ...
    output_fieldnames = original_fieldnames + [column for column in AI_COLUMNS if column not in original_fieldnames]
